...
with tabs[3]:
    ...
    if not st.session_state.get("control_approved"):
        ...
        if "insights" in st.session_state:
            ...
            total_exceptions = int(insights.get("exception_count", 0))
            ...
            with chart_col1:
                ...
                if not exception_breakdown.empty:
                    st.bar_chart(
                        exception_breakdown.set_index("category")["count"],
                        use_container_width=True,
                    )
                else:
                    st.info("No exception mix available.")
            ...
            with chart_col2:
                ...
                if not impact_breakdown.empty:
                    st.bar_chart(
                        impact_breakdown.set_index("category")["impact"],
                        use_container_width=True,
                    )
                else:
                    st.info("No financial impact breakdown available.")
            ...
            if not top_exceptions.empty:
                ...
                st.info("No top exception sample available.")
